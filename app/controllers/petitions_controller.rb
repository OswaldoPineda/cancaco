class PetitionsController < ApplicationController
  ...
  def create
    ...
    if @petition.save
      redirect_to categories_petitions_path, notice: I18n.t('success.messages.petition_published')
    else
  ...
  def valid_categories_and_subcategories
    ...
    if @category.nil?
      redirect_to categories_petitions_path, alert: I18n.t('errors.messages.category_not_found')
    else
      valid_subcategory
    end
  end
  ...
  def valid_subcategory
    ...
    if @subcategory.nil? && category_has_subcategory?
      redirect_to categories_petitions_path, alert: I18n.t('errors.messages.subcategory_not_found')
    end
  end
